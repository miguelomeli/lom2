...
#if defined(NDEBUG)
# error "Lomeli cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Lomeli Signed Message:\n";
...
{
    // Lomeli: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetSpacing = 6 * 60;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Lomeli: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Lomeli: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1525380046, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        assert(block.hashMerkleRoot == uint256("0xc56c2a83fb2603ac50070d2b3328b26056abf7eb01ffd32deb00724b3a3355eb"));
...
unsigned char pchMessageStart[4] = { 0x47, 0x7f, 0x2d, 0xdb }; // Lomeli: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LomeliMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LomeliMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LomeliMiner(CWallet *pwallet)
{
    printf("LomeliMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("lomeli-miner");
    ...
    try { loop {
        ...
        printf("Running LomeliMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LomeliMiner, pwallet));
}
