...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Lomeli " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"192.241.208.62", "192.241.208.62"},
    {"192.241.217.41", "192.241.217.41"},
    {NULL, NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Lomeli is probably already running."), addrBind.ToString().c_str());
        else
